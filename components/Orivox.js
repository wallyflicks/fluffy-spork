...
const CASE_DIFF_LABELS = {Easy:"Warm-up",Medium:"Judge Round",Hard:"Final Round"};
const diffLabel=(cat,diff)=>cat==="Case Competition"?(CASE_DIFF_LABELS[diff]||diff):diff;
          ...
          {screen==="home"&&(
            <div>
              ...
              <div className="card fadeUp d3" style={{padding:"clamp(20px,5vw,40px)",marginBottom:20}}>
                ...
                <div style={{marginBottom:36}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
                    <span className="fredoka" style={{fontSize:20}}>{cat==="Case Competition"?"Round":"Difficulty"}</span>
                    <Sparkle size={20} color="var(--orange)"/>
                  </div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                    {DIFFS.map(d=><button key={d} className={`chip ${diff===d?"active":""}`} onClick={()=>setDiff(d)}>{diffLabel(cat,d)}</button>)}
              ...
              <div className="fadeUp" style={{display:"flex",alignItems:"center",gap:16,marginBottom:32}}>
                ...
                <div>
                  <div style={{display:"flex",gap:8,marginBottom:8}}>
                    {activeCat!=="Custom"&&<span style={{background:DIFF_BG[activeDiff],color:DIFF_COLOR[activeDiff],border:`2px solid ${DIFF_COLOR[activeDiff]}50`,borderRadius:50,padding:"4px 14px",fontSize:13,fontFamily:"Fredoka",fontWeight:600}}>{diffLabel(activeCat,activeDiff)}</span>}
